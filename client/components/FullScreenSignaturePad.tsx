...
export function FullScreenSignaturePad({
  ...
  useEffect(() => {
    ...
    // Store original orientation for mobile devices
    ...
    if (isMobileDevice && screen.orientation && screen.orientation.lock) {
      screen.orientation.lock("landscape-primary").catch((err) => {
        ...
        // Fallback: Try to lock in any landscape mode
        if (screen.orientation.lock) {
          screen.orientation.lock("landscape").catch((err2) => {
            console.log("Any landscape orientation lock not supported:", err2);
          });
        }
      });
    }
    ...
    return () => {
      ...
      if (isMobile && screen.orientation) {
        if (originalOrientation && screen.orientation.lock) {
          screen.orientation.lock(originalOrientation as OrientationLockType).catch(() => {
  ...
  }, [isOpen, originalOrientation, isMobile]);
  ...
  // Function to restore original orientation
  const restoreOrientation = () => {
    if (isMobile && screen.orientation) {
      if (originalOrientation && screen.orientation.lock) {
        // Try to restore original orientation
        screen.orientation.lock(originalOrientation as OrientationLockType).catch((err) => {
          console.log("Could not restore original orientation:", err);
          // Fallback: unlock orientation
          if (screen.orientation.unlock) {
            screen.orientation.unlock();
          }
        });
      } else if (screen.orientation.unlock) {
        // Just unlock if we don't have original orientation
        screen.orientation.unlock();
      }
    }
  };
        ...
        <div>
          ...
          {isMobile && (
            <p className="text-sm opacity-90">
              📱 Device rotated for better signing experience
            </p>
          )}
        </div>
