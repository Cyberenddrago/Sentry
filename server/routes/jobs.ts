...
export const handleCreateJob: RequestHandler = (req, res) => {
  try {
    ...
    // Auto-assign job number if not provided
    let jobNumber = (jobData as any).jobNumber;
    if (!jobNumber) {
      // Find the highest existing job number and add 1
      const existingNumbers = jobs
        .map(j => j.jobNumber)
        .filter(num => num !== undefined && num !== null) as number[];
      jobNumber = existingNumbers.length > 0 ? Math.max(...existingNumbers) + 1 : 1;
    }
...
export const handleUpdateJob: RequestHandler = (req, res) => {
  try {
    ...
    // Validate job number if provided (only allow admin users to change job numbers)
    if ((updates as any).jobNumber !== undefined) {
      const newJobNumber = (updates as any).jobNumber;
      if (newJobNumber !== null && newJobNumber !== currentJob.jobNumber) {
        // Check for duplicate job numbers
        const existingJob = jobs.find(j => j.id !== id && j.jobNumber === newJobNumber);
        if (existingJob) {
          return res.status(400).json({ error: `Job number ${newJobNumber} is already in use` });
        }
      }
    }
