...
async function initializeForms() {
  try {
    ...
    // Ensure no duplicates by using a Map with form IDs as keys
    ...
    // Fallback to legacy forms with deduplication
    const formMap = new Map();
...
export const handleGetForms: RequestHandler = (req, res) => {
  try {
    ...
    const uniqueForms = Array.from(
      new Map(filteredForms.map(form => [form.id, form])).values()
    );
