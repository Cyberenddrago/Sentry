...
export const handleGenerateFormPDF: RequestHandler = async (req, res) => {
  try {
    ...
    // Set response headers with custom filename for noncompliance
    const filename = submission.formId === "noncompliance-form"
      ? "Noncompliance.pdf"
      : `${submission.formId}-submission-${submission.submissionNumber}.pdf`;
...
async function generateABSAPDF(submission: any): Promise<Uint8Array> {
  ...
  if (useTemplate) {
    try {
      ...
      // Handle rating fields from field-checkbox1 to field-checkbox13 (1-10 ratings)
      for (let i = 1; i <= 13; i++) {
        const rating = parseInt(data[`field-checkbox${i}`], 10);
        if (!isNaN(rating) && rating >= 1 && rating <= 10) {
          try {
            // Try to find the rating field and mark it with X
            const fieldName = `CheckBox${i}-${rating}`;
            const field = form.getFieldMaybe(fieldName);
            if (field) {
              form.getTextField(fieldName).setText("X");
              console.log(`Set ABSA rating field: ${fieldName} = X`);
            } else {
              console.warn(`ABSA rating field not found: ${fieldName}`);
            }
          } catch (error) {
            console.warn(`Error setting ABSA rating field ${i}:`, error);
          }
        }
      }
...
async function generateLiabilityPDF(submission: any): Promise<Uint8Array> {
  ...
  // Basic info using correct field names from predefined form
  form.getTextField("L_Date").setText(data["date"] || moment().format("Do,MM"));
  form.getTextField("L_Insurance").setText(data["insurance"] || "");
  form.getTextField("L_ClaimNumber").setText(data["claim_number"] || "");
  form.getTextField("C_Name").setText(data["client"] || "");
  form.getTextField("P_Name").setText(data["plumber"] || "");

  // Main Assessment Items - map to L1-L8 fields
  // Existing Pipes/Fittings to Increase/Decrease in Pressure
  form.getTextField("L1").setText(data["existing_pipes_fittings"] || "");
  form.getTextField("L2").setText(data["roof_entry"] || "");
  form.getTextField("L3").setText(data["geyser_enclosure"] || "");
  form.getTextField("L4").setText(data["wiring_electrical_alarm"] || "");
  form.getTextField("L5").setText(data["waterproofing"] || "");
  form.getTextField("L6").setText(data["pipes_not_secured"] || "");
  form.getTextField("L7").setText(data["not_listed_comments"] || "");
  form.getTextField("L8").setText(data["increase_decrease_pressure"] || "");

  // Before/After sections mapping
  // Water Hammer Before/After
  form.getTextField("P_KPABEFORE").setText(data["water_hammer_before"] || "");
  form.getTextField("P_KPAAFTER").setText(data["water_hammer_after"] || "");

  // Pressure Test Before/After
  form.getTextField("T_BEFORE").setText(data["pressure_test_before"] || "");
  form.getTextField("T_AFTER").setText(data["pressure_test_after"] || "");

  // General Comments
  form.getTextField("textarea_33bxdi").setText(data["general_comments"] || "");

  // Before/After fields mapping to Yes/No checkboxes where applicable
  const yesNoFields = [
    // Thermostat Setting
    { formYes: "WH_Yes", formNo: "WH_No", field: "thermostat_setting_before" },
    { formYes: "WHA_Yes", formNo: "WHA_No", field: "thermostat_setting_after" },

    // External Isolator
    { formYes: "EI_Yes", formNo: "EI_No", field: "external_isolator_before" },

    // Balanced System
    {
      formYes: "Balanced_System_Yes",
      formNo: "Balanced_System_YesBalanced_System_No",
      field: "balanced_system_before",
    },

    // Non Return Valve
    { formYes: "NRValve_Yes", formNo: "NRValve_No", field: "non_return_valve_before" },
  ];
  ...
  // Note: Signature handling removed as per user request
  console.log("Liability PDF generated without signature");
...
async function generateNoncompliancePDF(submission: any): Promise<Uint8Array> {
  const data = submission.data;
  const inputPath = path.join(__dirname, "../../public/forms/Noncompliance.pdf");

  const pdfDoc = await PDFDocument.load(await fs.readFile(inputPath));
  const form = pdfDoc.getForm();

  // Fill text fields - using correct field names from noncompliance form
  try {
    form.getTextField("C_Number").setText(data["claim_number"] || data["claimNumber"] || "");
    form.getTextField("C_FName").setText(data["client_name"] || data["clientName"] || "");
    form.getTextField("I_Name").setText(data["insurance_name"] || data["insuranceName"] || "");
    form.getTextField("Date").setText(data["date"] || moment().format("YYYY-MM-DD"));
    form.getTextField("Geyser_make").setText(data["geyser_make"] || data["geyserMake"] || "");
    form.getTextField("Geyser_Serial").setText(data["geyser_serial"] || data["serial"] || "");
    form.getTextField("Geyser_Code").setText(data["geyser_code"] || data["code"] || "");
    form.getTextField("P_Name").setText(data["staff_name"] || data["plumber"] || "");
  } catch (fieldError) {
    console.warn("Some noncompliance text fields not found in PDF:", fieldError);
  }

  // Handle noncompliance issues checkboxes
  try {
    const selectedIssues = data["selectedIssues"] || [];
    for (let i = 1; i <= 33; i++) {
      const fieldName = `Check Box${i}`;
      if (selectedIssues.includes(i)) {
        try {
          form.getTextField(fieldName).setText("X");
        } catch (checkboxError) {
          console.warn(`Noncompliance checkbox ${fieldName} not found in PDF`);
        }
      }
    }
  } catch (issuesError) {
    console.warn("Error processing noncompliance issues:", issuesError);
  }
